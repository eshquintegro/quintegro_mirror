  ...
  ...
      steps:
        - uses: actions/checkout@v3
          with:
            fetch-depth: 0
        - uses: eshquintegro/quintegro_mirror@master
          with:
...
You can exclude specific branches from mirroring using the `EXCLUDE_BRANCHES` parameter.
Provide a comma-separated list of branch names (e.g. `'develop,staging,test'`).
...
### Exclude branches from mirroring

You can exclude specific branches from being mirrored:
      ...
      steps:
        ...
        - uses: eshquintegro/quintegro_mirror@master
          with:
            ...
            GIT_USERNAME: spyoungtech
            GIT_PASSWORD: ${{ secrets.GIT_PASSWORD }}
            EXCLUDE_BRANCHES: 'develop,staging,test'
```

When `PUSH_ALL_REFS` is `false` and the current branch is in the exclusion list, the action will fail with an error.
